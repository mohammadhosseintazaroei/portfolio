export default function toEnglishDigits(num: string | number): string {
  ...
  let output = '';
  for (let ipos of num) {
    let faIndex = faDigits.indexOf(ipos);
    if (faIndex >= 0) {
      output += faIndex.toString();
      continue;
    }

    let arIndex = arDigits.indexOf(ipos);
    if (arIndex >= 0) {
...
export function wordifyfa(input: string | number, level = 0): string {
  if (input === null) {
    return '';
  }
  ...
  if (num < 10) {
    ...
    result +=
      wordifyfa(Math.floor(num / 1000000000000), level) + ' تریلیارد' + wordifyfa(num % 1000000000000, level + 1);
  }
